...
class ddim_config(BaseModel):
    """
    Configuration class for DDIM inversion.
...
class DDIM:
    """
    Class for DDIM inversion and image generation.

    Args:
        config (ddim_config): Configuration object.
        device (torch.device): Device to run computations on.
    """

    def __init__(self, config, device):
        """
        Initialize the DDIM inversion pipeline.
    ...
    def encode(
        ...
        sampling_time_fracture: int = 1.0,
        prompt: str = " ",
        negative_prompt: str = " ",
        ...
        Args:
            ...
            num_inference_steps (int): Number of inference steps.
            sampling_time_fracture (float): Sampling time fracture.
        ...
        xt = self.latent_encoder(
            image,
        )
        timesteps = (
            self.prepare_time_steps(
                num_inference_steps=num_inference_steps, end_time=sampling_time_fracture
            )
            .to(latents.dtype)
            .to(latents.device)
        )
        ...
        xt = torch.cat([xt] * 2) if do_classifier_free_guidance else xt
        for i, t in enumerate(timesteps):

            timestep = self.rescale_time_step(t).expand(xt.shape[0])
    ...
    def decode(
        ...
        xt,
        num_inference_steps: int = 50,
        sampling_time_fracture: float = 1.0,
        strength: float = 1.0,
        prompt: str = " ",
        negative_prompt: str = " ",
        ...
        Args:
            xt (torch.Tensor): Latent representations.
            num_inference_steps (int): Number of inference steps.
            sampling_time_fracture (float): Sampling time fracture.
            strength (float): Strength of the decoding process.
            prompt (str): Text prompt for guidance.
